...
describe('DiversityEngine', () => {
  ...
  const dummyCareer = (code: string, category: string, subDomain = 'sub'): CareerDocument => ({
    career_code: code,
  ...
  it('filters out overly similar careers and moves them to related', () => {
    const list: DiversityInput[] = [
      { career: dummyCareer('se', 'science', 'se_pcm'), score: 95, originalResult: {} },
      { career: dummyCareer('ds', 'science', 'se_pcm'), score: 94, originalResult: {} }, // too similar (same category and subdomain)
      { career: dummyCareer('doctor', 'health', 'med_pcb'), score: 90, originalResult: {} }, // different category
    ];
