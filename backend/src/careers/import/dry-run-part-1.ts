...
const filePath = path.resolve(__dirname, '../../../../SCPR_Master_Career_Catalog_Part_1_Science_v2.md');
...
for (const [code, count] of Object.entries(bySubDomain).sort((a, b) => b[1] - a[1])) {
...
for (const leaf of leaves.slice(0, 20)) {
  ...
  const { eligibility, needs_enrichment } = computeEligibility(categoryCode, subDomainCode);
  console.log(`  ${leaf.career_code}`);
  console.log(`    Name: ${leaf.name}`);
  console.log(`    Sub-domain: ${subDomainCode} (from: "${leaf.sub_domain_source}")`);
  console.log(`    Pathway: [${leaf.pathway_tags.join(', ')}]`);
  console.log(`    Traits: AT=${weights.analytical_thinking} CR=${weights.creativity} TC=${weights.technical_curiosity}`);
  console.log(`    Eligibility: math=${eligibility.min_maths} sci=${eligibility.min_science} stream=${eligibility.required_stream}`);
...
const importantCodes = [
  'software_engineer', 'ai_engineer', 'data_scientist',
  'mechanical_engineer', 'civil_engineer', 'architect',
  'general_physician', 'surgeon',
