...
import { computeEligibility, EligibilityConstraints } from './default-eligibility.config';
import { CATALOG_TO_CATEGORY, isValidSubDomain } from './taxonomy.config';
import { parseCatalogFile, ParsedCareerLeaf, computeSubDomainCode, slugify } from './tree-parser.service';
...
export class CareerSeedService {
  ...
  async seedFromCatalog(filePath: string, catalogPart: string): Promise<SeedPhaseResult> {
    ...
    if (!categoryCode) {
      throw new Error(`Unknown catalog part: ${catalogPart}. Must be one of: ${Object.keys(CATALOG_TO_CATEGORY).join(', ')}`);
    }
    ...
    try {
      ...
      throw new Error(`Failed to read catalog file at ${filePath}: ${err.message}`);
    }

    // Parse the ASCII tree
    const { leaves, anomalies, overview_skipped } = parseCatalogFile(content, catalogPart);
    ...
    if (leaves.length === 0) {
      return {
        ...
        overview_skipped,
        anomalies: anomalies.length > 0 ? anomalies : ['No career leaves found in catalog'],
        timestamp: new Date().toISOString(),
      };
    }

    this.logger.log(`Parsed ${leaves.length} career leaves from ${catalogPart}`);
    ...
    if (polytechnicLeaves.length > 0) {
      this.logger.log(`Cross-linking ${polytechnicLeaves.length} Polytechnic branches with Diploma careers...`);
      ...
      this.logger.log(`Cross-linking complete for ${polytechnicLeaves.length} Polytechnic branches`);
    }
    ...
    for (const leaf of careerLeaves) {
      ...
      const innerCode = parenMatch ? slugify(parenMatch[1].trim().replace(/\./g, '_')) : null;
      ...
      const existing = await this.careerModel.findOne({ career_code: leaf.career_code }).exec();
      ...
      if (existing) {
        ...
        const { eligibility, needs_enrichment: enrichmentFlag } = computeEligibility(categoryCode, subDomainCode);
        ...
        const broadDegreeKeywords = [
          'engineering', 'bachelor', 'master', 'b.sc', 'b.com', 'b.a', 'm.sc',
          'm.com', 'm.a', 'b.tech', 'm.tech', 'b.e', 'm.e', 'm.b.a',
          'b.b.a', 'b.c.a', 'm.c.a', 'b.pharma', 'b.arch', 'b.des',
          'b.f.a', 'b.p.ed', 'b.ed', 'm.ed',
        ];
        ...
        const newCareer = new this.careerModel({
          ...
          trait_weights: traitWeights as Career['trait_weights'],
          eligibility: {
    ...
    if (mergedDuplicates > 0) {
      this.logger.log(`Merged ${mergedDuplicates} duplicates from ${catalogPart}`);
    }
