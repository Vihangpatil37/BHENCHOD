...
const fixtures = {
  ...
  roadmap_generation: {
    valid: {
      ...
      phases: [{ phase: 'School', duration: '2y', action_items: ['Study'] }],
    },
...
describe('validate', () => {
  describe.each(Object.entries(fixtures))('%s', (taskType, f) => {
    it('accepts valid data', () => {
      expect(() =>
        service.validateAndRepair(JSON.stringify(f.valid), taskType),
      ).not.toThrow();
    });
    ...
    it('rejects missing required field', () => {
      expect(() =>
        service.validateAndRepair(
          JSON.stringify(f.missingField as any),
          taskType,
        ),
    ...
    it('rejects wrong field type', () => {
      expect(() =>
        service.validateAndRepair(JSON.stringify(f.wrongType as any), taskType),
      ).toThrow(BadRequestException);
    });
  });
});
...
describe('repair', () => {
  ...
  it('fails fast on truncated JSON', () => {
    expect(() =>
      service.validateAndRepair('{"reply": "hel', 'counselor_chat'),
    ).toThrow(BadRequestException);
  });
