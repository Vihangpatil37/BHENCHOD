...
async function bootstrap() {
  ...
  const recModel = app.get<Model<Recommendation>>(
    getModelToken(Recommendation.name),
  );
  const careerModel = app.get<Model<Career>>(getModelToken(Career.name));
  const msgModel = app.get<Model<ConversationMessage>>(
    getModelToken(ConversationMessage.name),
  );
  const convModel = app.get<Model<Conversation>>(
  ...
  for (const r of recs) {
    if (!r.final_recommendations || r.final_recommendations.length === 0)
      continue;
  ...
  console.log(
    `  ${recs.length} docs, ${all.filter((e) => e.source === 'Recommendation').length} failures`,
  );

  // 2. Careers (ai_refined / published)
  console.log('Querying Careers...');
  const careers = await careerModel
    .find({ backfill_status: { $in: ['ai_refined', 'published'] } })
    .lean()
    .exec();
  ...
  const convs = await convModel
    .find({ summary: { $ne: '', $exists: true } })
    .lean()
    .exec();
  ...
  if (all.length === 0) {
    lines.push(
      '✅ All stored documents pass validation against the corrected schemas.',
    );
  } else {
    lines.push('## Per-Document Failures');
    lines.push('');
    lines.push(
      '> Read-only diagnostic. Do not auto-correct or delete any document.',
    );
    lines.push('');
    for (const [taskType, entries] of byType) {
      ...
      for (const e of entries) {
        ...
        lines.push(
          `| \`${e._id}\` ${dateStr ? `(${dateStr})` : ''} | ${e.source} | ${e.errors.join('; ')} |`,
        );
      }
      lines.push('');
    }
    ...
    const preFixEntries = all.filter(
      (e) => e.createdAt && e.createdAt < '2026-07-01',
    );
    if (preFixEntries.length > 0) {
      lines.push('## Pre-Fix Backfill Data');
      lines.push('');
      lines.push(
        `The following ${preFixEntries.length} entries were created before the validator fix (approximate cutoff: 2026-07-01):`,
      );
      lines.push('');
      for (const e of preFixEntries) {
        lines.push(
          `- \`${e._id}\` (${e.taskType}, ${e.createdAt?.substring(0, 10)}): ${e.errors.join('; ')}`,
        );
      }
      lines.push('');
    }
