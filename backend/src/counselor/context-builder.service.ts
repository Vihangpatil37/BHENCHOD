...
export class ContextBuilderService {
  ...
  async buildContext(
    ...
    this.logger.log(
      `Building chat context for conversation: ${conversation._id}`,
    );

    // 1. Roll-up and summarize if history threshold is exceeded (> 10 messages)
    if (messages.length > 10) {
      this.logger.log(
        `Conversation messages length (${messages.length}) exceeds threshold (10). Running rolling compression...`,
      );
      ...
      const textToSummarize = messagesToSummarize
        .map(
          (m) =>
            `${m.role === 'student' || m.role === 'user' ? 'Student' : 'Counselor'}: ${m.content}`,
        )
        .join('\n');
      ...
      try {
        ...
        if (summaryResponse.success && summaryResponse.data) {
          ...
          this.logger.log(
            `Successfully updated rolling summary for conversation: ${conversation._id}`,
          );
        ...
        this.logger.error(
          `Failed to compress conversation history: ${err.message}. Proceeding with full history.`,
        );
      }
    }
    ...
    const candidateCareersStr =
      'No recommended careers available yet. The student has not completed onboarding recommendations.';
